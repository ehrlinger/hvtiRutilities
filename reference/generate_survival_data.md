...
`rf.survival.study.qmd`. Variable labels are attached as
`attr(..., "label")` so they are compatible with `haven` and
...
A `data.frame` with `n` rows and 22 columns:
...
`model_data` in the template (`ccfid`, `reop`, `iv_reop`), plus the
survival outcome (`iv_dead`, `dead`) and 17 clinical predictors spanning
demographics, pre-operative labs, cardiac function, and surgical
variables.
...
#>   0.250   2.439   4.100   4.888   6.612  13.980 
