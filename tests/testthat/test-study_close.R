...
test_that("a fresh copy with a remote sees the closure history", {
  skip_if_no_git()
  local_git_env()
  dir <- withr::local_tempdir()
  bare <- make_bare_remote(dir)
  a <- make_checkpoint_study(withr::local_tempdir())
  set_study_keys(a, checkpoint = list(remote = bare))
  suppressMessages(study_checkpoint("manuscript_published", root = a))
  expect_equal(study_close("abandoned", root = a)$delivery$git, "delivered")
  b <- make_checkpoint_study(withr::local_tempdir())
  set_study_keys(b, checkpoint = list(remote = bare))
  expect_false(dir.exists(file.path(b, ".checkpoint")))
  expect_error(study_close("not_published", root = b), "already closed")
  ro <- suppressMessages(study_reopen("new PI", root = b))
  expect_equal(ro$tag, "reopened-1")
  cl <- study_close("published", publication = pub, root = b)
  expect_equal(cl$tag, "closed-published-2")
  expect_equal(cl$delivery$git, "delivered")
})

test_that("a pushed closure tag is never renumbered", {
  skip_if_no_git()
  local_git_env()
  dir <- withr::local_tempdir()
  root <- make_checkpoint_study(dir)
  ...
  other <- file.path(dir, "other")
  git_out(dir, c("clone", "-q", bare, other))
  git_out(other, c("tag", "-a", "closed-published-1", "-m", "other",
                   "origin/main"))
  git_out(other, c("push", "-q", "origin", "refs/tags/closed-published-1"))
  expect_true("closed-published-1" %in% git_out(bare, c("tag", "-l")))
  # The push landed but its delivery was never recorded.
  log <- .cp_log_read(root)
  log[[1]]$delivery$git <- "pending"
  .cp_log_write(root, log)
  study_checkpoint_push(root)
  e <- .cp_log_read(root)[[1]]
  expect_equal(e$tag, "closed-abandoned-1")
