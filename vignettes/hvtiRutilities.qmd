...
- **`read_clinical_data()`**: Read SAS, CSV, Excel, or RDS files with automatic type conversion
...
Use `read_clinical_data()` to read and convert in one step:
...
`.rds` files. Set `convert_types = FALSE` to skip automatic type conversion.
...
- **`read_clinical_data()`**: One-step import from SAS, CSV, Excel, or RDS
